...
def _validate_price_range(price: Optional[float], max_price: Optional[float]) -> None:
    """
    price = min price (required on create, optional on update)
    max_price = optional upper range
    """
    if price is not None and price < 0:
        raise HTTPException(status_code=400, detail="Price cannot be negative")
    if max_price is not None and max_price < 0:
        raise HTTPException(status_code=400, detail="Max price cannot be negative")
    if price is not None and max_price is not None and max_price < price:
        raise HTTPException(status_code=400, detail="max_price must be >= price")
...
async def create_product(
    ...
    # ✅ validate range
    _validate_price_range(data.price, getattr(data, "max_price", None))
    ...
    product = Product(
        ...
        max_price=getattr(data, "max_price", None),  # ✅ NEW
        original_price=data.original_price,
        discount_percentage=data.discount_percentage or 0.0,
        stock=data.stock,
        image_url=data.image_url,
...
async def update_product(
    ...
    # ✅ price + max_price (range)
    if getattr(data, "price", None) is not None:
        product.price = data.price

    if hasattr(data, "max_price") and data.max_price is not None:
        product.max_price = data.max_price

    # If client wants to clear max_price (make fixed price again),
    # they can send "max_price": null explicitly.
    if hasattr(data, "max_price") and data.max_price is None:
        # only clear if the field exists in the schema payload (it does)
        # NOTE: FastAPI/Pydantic includes it as None if sent or defaulted.
        # If you want "clear only when explicitly sent", see note below.
        pass

    _validate_price_range(product.price, product.max_price)
